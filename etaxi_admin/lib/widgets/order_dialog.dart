...
class _OrderDialogState extends State<OrderDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Column(
        children: [
          ...
          SearchBar(
            hintText: 'Pocetna lokacija',
            controller: TextEditingController(text: notifier.currentLocationData?.address ?? null),
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: ((context) => PlacePickerWidget(
                  onPicked: (pickedData){
                  
                    OrderProvider.instance.setCurrentLoc(Location.fromPickedData(pickedData));
          ...
          SizedBox(height: 16,),
           Consumer<OrderProvider>(
            builder: (context, notifier, child) => 
          SearchBar(
            hintText: 'Finalna lokacija',
            controller: TextEditingController(text: notifier.destinationLocationData?.address ?? null),
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: ((context) => PlacePickerWidget(
                  onPicked: (pickedData){
                  
                    OrderProvider.instance.setDestinationLoc(Location.fromPickedData(pickedData));
                    Navigator.pop(context);
                  },
                ))));
              }
    ...
    Spacer(),
    CustomButton(label: 'Naruci voznju',onPressed: (){Navigator.pop(context);},)
        ],
      ),
    );
  }
